...
I comandi conversazionali sono istruzioni che dai all'agente nella chat, precedute dal prefisso \texttt{@}. Non sono comandi shell. Sono segnali semantici che l'agente riconosce e interpreta secondo il protocollo AI-DLC.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lll}
    ...
    \midrule
    \texttt{@Task [obiettivo]} & Crea e avvia un task & Propone task + test (no auto) \\
    ...
    \bottomrule
  \end{tabular}
  \caption{Comandi conversazionali AI-DLC}
...
Il comando con cui inizia l'implementazione. Crea l'unità di lavoro (il task) e ne avvia il ciclo test-driven descritto nel Capitolo~\ref{ch:fasi-e-modes} (§\ref{sec:06-task-tdd}). Input: un obiettivo, oppure un riferimento a un Epic o a un requisito.

\begin{lstlisting}[language=,caption={Output tipico di @Task}]
T-007 — POST /tasks
...
In \texttt{.ai-dlc/project/instructions.md}:
...
\begin{itemize}
  ...
  \item \texttt{@Task} crea e avvia l'unità di implementazione (sizing, AC, test) e ne governa il ciclo test-driven.
  ...
  \item Comandi personalizzati in \texttt{.ai-dlc/project/instructions.md} trasformano workflow ricorrenti in shortcut.
\end{itemize}
