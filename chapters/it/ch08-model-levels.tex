...
C'è un errore comune nell'uso degli agenti AI: usare sempre il modello più potente disponibile. Il primo motivo è il costo: i modelli più potenti costano di più in token. Il secondo è la latenza: su task semplici e frequenti la differenza è percepibile e non giustificata.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{cll}
    ...
    \bottomrule
  \end{tabular}
  \caption{Scala dei Model Level AI-DLC}
  \label{tab:08-scala}
\end{table}

Il mapping ai modelli concreti (Anthropic, OpenAI, Gemini) vive in \texttt{.ai-dlc/manifest.json\#model\_levels} e si legge con:

\begin{lstlisting}[language=,caption={Visualizzazione mapping modelli}]
bash .ai-dlc/tools/show-models.sh   # Linux/macOS/WSL
.\.ai-dlc\tools\show-models.ps1     # Windows PowerShell
\end{lstlisting}
...
Il task è classificato MEDIUM. Il risk floor per MEDIUM è livello 3. Anche se la stima token suggerisce livello 2, il livello sale a 3.
...
\begin{itemize}
  ...
  \item Il risk floor forza un livello minimo indipendentemente dalla stima token.
