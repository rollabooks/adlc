...
Use `.adlc/modules/templates/CONTEXT_TEMPLATE.md` for the complete version, `CONTEXT_MIN.md` for spikes/POCs.
...
**Modules** (`.adlc/modules/00_MODE.md`, `01_CORE_RULES.md`, etc.) are the framework. Read-only. Loaded only when needed for the current phase.
...
`.adlc/halt-triggers.yaml` defines file patterns that require **explicit confirmation** before modification: database schema, authentication code, secrets, infrastructure, CI/CD, framework files.

When the agent touches one of these paths, it stops and asks before proceeding. You can override the default by creating `.adlc/project/halt-triggers.yaml` in your project.
...
The vendor mapping (Anthropic/OpenAI/Gemini) for each level lives in `.adlc/manifest.json#model_levels`. To print it:
...
For small projects or spikes use `--context minimal`. Creates: `_CONTEXT.md`, `PROGRESS.md`, `.adlc/project/instructions.md`, `.adlc/project/skills/` folder.
...
All in `.adlc/tools/`. Work in both PowerShell (`.ps1`) and Bash (`.sh`).
...
|--------|-------|---------|
| `.adlc/modules/09_CODEBASE_ANALYSIS.md` | Intake | Understand structure, dependencies, hotspots, risks |
| `.adlc/modules/10_DOCUMENTATION.md` | Output | Produce navigable Markdown from templates |
...
```
Load .adlc/modules/09_CODEBASE_ANALYSIS.md and analyze this repository.
...
```
Load .adlc/modules/10_DOCUMENTATION.md and generate documentation
...
Each subproject has its own `_CONTEXT.md`, `PROGRESS.md`, `.adlc/project/`. The agent uses the `_CONTEXT.md` closest to the file it's working on.
...
If your project must follow enterprise SDLC processes, governance, compliance, or internal standards, create `.adlc/company/`. The agent loads it automatically.
...
Check `.adlc/halt-triggers.yaml`: is the affected path covered? If not, add it as an override in `.adlc/project/halt-triggers.yaml`.
...
Epic must have `## Risk and Model Floor` and `## Task Breakdown`. Use `.adlc/modules/templates/EPIC_TEMPLATE.md` as reference.
...
### "I want to modify `.adlc/modules/...`"
Don't. Modules are read-only during normal use. For customizations, put rules in `.adlc/project/instructions.md` or skills in `.adlc/project/skills/`.
...
Don't circumvent it. Either change the affected path in `.adlc/project/halt-triggers.yaml` (remove that pattern from the trigger) or accept that the path requires confirmation — it's there for good reasons.
...
- **Company extension**: `.adlc/company/` folder with additional enterprise rules.
...
- **Module**: file `.adlc/modules/NN_*.md`, framework rule loaded per phase.
...
- **Skill**: file `.adlc/modules/skills/SKILL_*.md`, thematic guide loaded on demand.
...
- `.adlc/INSTALL.md` — setup checklist
- `.adlc/COMMANDS.md` — full conversational commands
- `.adlc/modules/01_CORE_RULES.md` — detailed base rules
- `.adlc/modules/SEC_CONSTRAINTS.md`, `PERF_CONSTRAINTS.md` — constraint libraries
- `.adlc/examples/` — populated examples of context, epic, task, progress
- `.adlc/VERSIONING.md` — versioning policy
