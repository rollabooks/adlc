...
$sourceDir = Join-Path $ProjectRoot ".adlc/company/source"
$processedDir = Join-Path $ProjectRoot ".adlc/company/processed"
...
if (-not (Test-Path -LiteralPath $sourceDir)) {
  ...
  Write-Host "Created: .adlc/company/source"
}

if (-not (Test-Path -LiteralPath $processedDir)) {
  New-Item -ItemType Directory -Force -Path $processedDir | Out-Null
  Write-Host "Created: .adlc/company/processed"
}
...
$index.Add("")
$index.Add("> Generated from `.adlc/company/source/`.")
$index.Add("")
...
foreach ($file in $sources) {
  ...
  if ((Test-Path -LiteralPath $target) -and -not $Force) {
    ...
    $documents.Add([ordered]@{
      source = ".adlc/company/source/$($file.Name)"
      output = ".adlc/company/processed/$safe.md"
      status = $status
      tool = $tool
      message = $message
    })
