...
usage() {
  cat <<'EOF'
Usage: bash .adlc/tools/update-projects.sh [--repo-root PATH]

Scans for _CONTEXT.md files and regenerates .adlc/projects.json.
EOF
}
...
mkdir -p "$REPO_ROOT/.adlc"
output="$REPO_ROOT/.adlc/projects.json"
tmp="$(mktemp)"
...
for root, dirs, files in os.walk(repo):
    ...
    projects.append({
        ...
        "project_rules": prefix + ".adlc/project",
        "company_extension": prefix + ".adlc/company",
    })
...
print(f"Updated: .adlc/projects.json ({len(projects)} projects)")
PY
...
print("Updated: .adlc/projects.json (%s projects)" % len(projects))
PY
else
