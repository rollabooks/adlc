...
usage() {
  cat <<'EOF'
Usage: bash .adlc/tools/preprocess-company-docs.sh [--project-root PATH] [--force]

Converts .adlc/company/source/ DOCX/PDF/TXT/MD files into
.adlc/company/processed/ Markdown files for agent consumption.
...
SOURCE_DIR="$PROJECT_ROOT/.adlc/company/source"
PROCESSED_DIR="$PROJECT_ROOT/.adlc/company/processed"
...

{
  ...
  printf '> Generated from `.adlc/company/source/`.\n\n'
} > "$INDEX"
...
for file in "$SOURCE_DIR"/*; do
  ...
  if [[ -e "$target" && "$FORCE" -ne 1 ]]; then
    ...
    add_manifest_item ".adlc/company/source/$filename" ".adlc/company/processed/$safe.md" "skipped" "" "Skipped existing"
  ...
  case "$ext" in
    docx)
      if ! command -v pandoc >/dev/null 2>&1; then
        ...
        add_manifest_item ".adlc/company/source/$filename" "" "failed" "" "Cannot process DOCX without pandoc"
        continue
      fi
      pandoc "$file" -t gfm -o "$target"
      add_report "Processed DOCX: $filename -> $safe.md"
      add_manifest_item ".adlc/company/source/$filename" ".adlc/company/processed/$safe.md" "processed" "pandoc" "Processed DOCX"
    ...
    pdf)
      if command -v pandoc >/dev/null 2>&1 && pandoc "$file" -t gfm -o "$target" 2>/dev/null && [[ -s "$target" ]]; then
        ...
        add_manifest_item ".adlc/company/source/$filename" ".adlc/company/processed/$safe.md" "processed" "pandoc" "Processed PDF"
        printf -- '- [%s.md](%s.md) — from `%s`\n' "$safe" "$safe" "$filename" >> "$INDEX"
        continue
      fi
      ...
      if command -v pdftotext >/dev/null 2>&1; then
        ...
        add_manifest_item ".adlc/company/source/$filename" ".adlc/company/processed/$safe.md" "processed" "pdftotext" "Processed PDF"
        printf -- '- [%s.md](%s.md) — from `%s`\n' "$safe" "$safe" "$filename" >> "$INDEX"
        continue
      fi

      add_report "Cannot process PDF without pandoc or pdftotext: $filename"
      add_manifest_item ".adlc/company/source/$filename" "" "failed" "" "Cannot process PDF without pandoc or pdftotext"
      ;;
    md|txt)
      cp -f "$file" "$target"
      add_report "Copied text document: $filename -> $safe.md"
      add_manifest_item ".adlc/company/source/$filename" ".adlc/company/processed/$safe.md" "processed" "copy" "Copied text document"
      printf -- '- [%s.md](%s.md) — from `%s`\n' "$safe" "$safe" "$filename" >> "$INDEX"
      ;;
    *)
      add_report "Unsupported source format: $filename"
      add_manifest_item ".adlc/company/source/$filename" "" "unsupported" "" "Unsupported source format"
      ;;
  esac
done
