...
Vai con `.adlc/modules/templates/CONTEXT_TEMPLATE.md` per la versione completa, `CONTEXT_MIN.md` per spike/POC.
...
I **moduli** (`.adlc/modules/00_MODE.md`, `01_CORE_RULES.md`, ecc.) sono il framework. Read-only. Si caricano solo quando servono per la fase corrente.
...
`.adlc/halt-triggers.yaml` definisce pattern di file che richiedono **conferma esplicita** prima di essere modificati: schema DB, codice di autenticazione, secrets, infrastruttura, CI/CD, framework files.

L'agente quando tocca uno di questi path si ferma e ti chiede prima di procedere. Puoi sovrascrivere il default mettendo `.adlc/project/halt-triggers.yaml` nel tuo progetto.
...
Il mapping vendor (Anthropic/OpenAI/Gemini) per ogni livello vive in `.adlc/manifest.json#model_levels`. Per stamparlo:
...
Lista completa: `.adlc/COMMANDS.md`.
...
Per progetti piccoli o spike usa `--context minimal`. Crea: `_CONTEXT.md`, `PROGRESS.md`, `.adlc/project/instructions.md`, cartella `.adlc/project/skills/`.
...
Tutti in `.adlc/tools/`. Funzionano sia in PowerShell (`.ps1`) sia in Bash (`.sh`).
...
| Modulo | Fase | Scopo |
|--------|------|-------|
| `.adlc/modules/09_CODEBASE_ANALYSIS.md` | Intake | Capire struttura, dipendenze, hotspot, rischi |
| `.adlc/modules/10_DOCUMENTATION.md` | Output | Produrre Markdown navigabile da template |
...
```
Carica .adlc/modules/09_CODEBASE_ANALYSIS.md e analizza questo repository.
...
Ogni sottoprogetto ha il suo `_CONTEXT.md`, `PROGRESS.md`, `.adlc/project/`. L'agente usa il `_CONTEXT.md` più vicino al file su cui sta lavorando.
...
Se il progetto deve seguire processi SDLC aziendali, governance, compliance, standard interni, crea `.adlc/company/`. L'agente lo carica automaticamente.
...
Controlla `.adlc/halt-triggers.yaml`: il path interessato è coperto? Se no, aggiungilo come override in `.adlc/project/halt-triggers.yaml`.
...
L'epic deve avere `## Risk and Model Floor` e `## Task Breakdown`. Usa `.adlc/modules/templates/EPIC_TEMPLATE.md` come riferimento.
...
### "Voglio modificare `.adlc/modules/...`"
Non farlo. I moduli sono read-only durante l'uso normale. Per personalizzazioni metti regole in `.adlc/project/instructions.md` o skill in `.adlc/project/skills/`.
...
Non scavalcare. Cambia il path interessato in `.adlc/project/halt-triggers.yaml` (rimuovi quel pattern dal trigger) o accetta che il path richiede conferma — è lì per buoni motivi.
...
- **Company extension**: cartella `.adlc/company/` con regole aziendali aggiuntive.
...
- **Module**: file `.adlc/modules/NN_*.md`, regola framework caricata per fase.
...
- **Skill**: file `.adlc/modules/skills/SKILL_*.md`, guida tematica caricata su richiesta.
...
- `.adlc/INSTALL.md` — checklist setup
- `.adlc/COMMANDS.md` — comandi conversazionali completi
- `.adlc/modules/01_CORE_RULES.md` — regole base dettagliate
- `.adlc/modules/SEC_CONSTRAINTS.md`, `PERF_CONSTRAINTS.md` — librerie di vincoli
- `.adlc/examples/` — esempi compilati di context, epic, task, progress
- `.adlc/VERSIONING.md` — policy di versioning
