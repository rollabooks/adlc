...
try {
  ...
  & (Join-Path $Root ".ai-dlc/tools/init.ps1") -ProjectRoot $project -FrameworkRoot $Root -Context minimal -Company
  Assert-Path (Join-Path $project "_CONTEXT.md")
  Assert-Path (Join-Path $project "PROGRESS.md")
  Assert-Path (Join-Path $project ".ai-dlc/project/instructions.md")
  Assert-Path (Join-Path $project ".ai-dlc/company/README.md")

  Set-Content -LiteralPath (Join-Path $project ".ai-dlc/company/source/PROCESS.txt") -Value "Gate: security review" -Encoding utf8
  & (Join-Path $Root ".ai-dlc/tools/preprocess-company-docs.ps1") -ProjectRoot $project
  Assert-Path (Join-Path $project ".ai-dlc/company/processed/INDEX.md")
  Assert-Path (Join-Path $project ".ai-dlc/company/processed/manifest.json")

  & (Join-Path $Root ".ai-dlc/tools/update-projects.ps1") -RepoRoot $tmp
  Assert-Path (Join-Path $tmp ".ai-dlc/projects.json")

  & (Join-Path $Root ".ai-dlc/tools/validate.ps1") -Root $Root
  ...
  $required = @(
    ...
    ".ai-dlc/AI-DLC.md", ".ai-dlc/INSTALL.md", ".ai-dlc/VERSIONING.md", ".ai-dlc/VERSION",
    ".ai-dlc/manifest.json",
    ".ai-dlc/tools/init.ps1", ".ai-dlc/tools/init.sh",
    ".ai-dlc/tools/preprocess-company-docs.ps1", ".ai-dlc/tools/preprocess-company-docs.sh",
    ".ai-dlc/tools/update-projects.ps1", ".ai-dlc/tools/update-projects.sh",
    ".ai-dlc/tests/test.ps1", ".ai-dlc/tests/test.sh",
    ".ai-dlc/schemas/manifest.schema.json", ".ai-dlc/schemas/projects.schema.json",
...
"@
  $exit = 0
  try {
    & (Join-Path $Root ".ai-dlc/tools/validate.ps1") -Root $negativeRoot 2>$null
  } catch { $exit = 1 }
...
"@
  ...
  & (Join-Path $Root ".ai-dlc/tools/validate.ps1") -Root $negativeRoot -Strict 2>$null
  if ($LASTEXITCODE -eq 0) {
