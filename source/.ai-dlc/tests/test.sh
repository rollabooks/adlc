...
bash "$ROOT/.ai-dlc/tools/init.sh" --project-root "$project" --framework-root "$ROOT" --context minimal --company
assert_path "$project/_CONTEXT.md"
assert_path "$project/PROGRESS.md"
assert_path "$project/.ai-dlc/project/instructions.md"
assert_path "$project/.ai-dlc/company/README.md"

printf 'Gate: security review\n' > "$project/.ai-dlc/company/source/PROCESS.txt"
bash "$ROOT/.ai-dlc/tools/preprocess-company-docs.sh" --project-root "$project"
assert_path "$project/.ai-dlc/company/processed/INDEX.md"
assert_path "$project/.ai-dlc/company/processed/manifest.json"

bash "$ROOT/.ai-dlc/tools/update-projects.sh" --repo-root "$TMP_DIR"
assert_path "$TMP_DIR/.ai-dlc/projects.json"

bash "$ROOT/.ai-dlc/tools/validate.sh" "$ROOT"
...
EOF
if bash "$ROOT/.ai-dlc/tools/validate.sh" "$negative_root" >/dev/null 2>&1; then
...
if bash "$ROOT/.ai-dlc/tools/validate.sh" "$negative_root" --strict >/dev/null 2>&1; then
