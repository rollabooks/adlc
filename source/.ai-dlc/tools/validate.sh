...
if [[ -f "$manifest" ]]; then
  if command -v jq >/dev/null 2>&1; then
    if ! jq -e '.version' "$manifest" >/dev/null 2>&1; then
      add_failure "Invalid .ai-dlc/manifest.json or missing version"
    fi
...
validate_schema_if_available "$ROOT/.ai-dlc/manifest.json" "$ROOT/.ai-dlc/schemas/manifest.schema.json" "manifest.json"
validate_schema_if_available "$ROOT/.ai-dlc/projects.json" "$ROOT/.ai-dlc/schemas/projects.schema.json" "projects.json"

while IFS= read -r -d '' file; do
  rel="${file#$ROOT/}"
  case "$rel" in
    .git/*|CHANGELOG.md|.ai-dlc/MIGRATION.md|.ai-dlc/tools/validate.ps1|.ai-dlc/tools/validate.sh)
      continue
      ;;
  esac
...
halt_file="$ROOT/.ai-dlc/halt-triggers.yaml"
if [[ -f "$halt_file" ]]; then
  if ! grep -Eq '^version:[[:space:]]+[0-9]+' "$halt_file"; then
    add_failure ".ai-dlc/halt-triggers.yaml missing version field"
  fi
  if ! grep -Eq '^triggers:' "$halt_file"; then
    add_failure ".ai-dlc/halt-triggers.yaml missing triggers list"
  fi
fi
