...
- Use **plan mode** (`Shift+Tab` or `/plan`) before touching any path in `.ai-dlc/halt-triggers.yaml`
  or anything marked HIGH in `_CONTEXT.md`.
- Prefer neutral project rules in `.ai-dlc/project/`; keep `.copilot/` compatibility when present.
- Load `.ai-dlc/company/` if present for enterprise process gates and standards.
- Load framework modules from `.ai-dlc/modules/` only as needed for the active phase.
